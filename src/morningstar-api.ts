...
export class MorningstarAPI {
  ...
  /**
   * Récupère les données de base d'un titre (Fonds ou Action) via l'API ecint.
   */
  async getSecurityData(isin: string): Promise<SecurityDataResponse | null> {
    const token = await this.getBearerToken();

    const url = `${this.baseUrl}/securities/${isin}`;
    const params = {
      idtype: "ISIN",
      viewid: "ITsnapshot", // This view works for both funds and stocks to get the basic type
      currencyId: "EUR",
    ...
    try {
      ...
      console.log(`  > ecint API returned empty for ${isin}, trying website search fallback...`);
      return this.findSecidFromWebsite(isin);
    } catch (error) {
      if ((error as AxiosError).response?.status === 401) {
        // This can happen for stocks not covered by this endpoint. Try the fallback.
        console.log(`  > ecint API failed for ${isin}, trying website search fallback...`);
        return this.findSecidFromWebsite(isin);
      }
      console.warn(`  Warning: Could not fetch data for ${isin} from ecint API. Error: ${(error as Error).message}`);
      return null;
    }
  }
